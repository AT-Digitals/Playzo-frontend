...
export default function TurfBooking() {
  ...
  const handleBreadcrumbClick = (breadcrumbKey: any) => {
    ...
    return (
      ...
      <Box margin={"60px 50px"} border={"1px solid #D9D9D9"} width={"100%"} maxWidth={"200px"} borderRadius={"10px"} height={"195px"}>
      <Box padding={"20px 20px"}>
      <img src={turf} width={"160px"} height={"130px"} alt="turf" />
      <Typography textAlign={"center"} fontSize={"14px"} color={Colors.BLACK} fontWeight={"600"}>Turf</Typography>
      </Box>
      </Box>
      <Stack borderLeft={"1px solid #D9D9D9"} width={"100%"} gap={"8px"} padding={"40px 37px"} flexDirection={"column"}>
      <Typography fontSize={"14px"} color={Colors.BLACK} fontWeight={"500"}>Choose Turf</Typography>
      {TurfImages.map((item) => (
      <Box sx={{
        ":hover": {
